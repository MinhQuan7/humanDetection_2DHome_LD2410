  ...
  void loop()
  {
    ...
    if(radar.isConnected() && millis() - lastReading > 1000)  //Report every 1000ms
    {
      ...
      if(radar.presenceDetected())
      {
        ...
        ERa.virtualWrite(V7,45);
        ERa.virtualWrite(V8,32);
        ERa.virtualWrite(V9,85);
        }
        if(radar.movingTargetDetected())
        {
          ...
          ERa.virtualWrite(V5,LOW);
        ERa.virtualWrite(V7,50);
        ERa.virtualWrite(V8,20);
        ERa.virtualWrite(V9,96);
          }
        Serial.println();
      }
      else
